...
struct AvadaKevadra {
    ...
    :
        tag(tag), parent(parent), name(name), count(count)
    {}
...
inline TagTreeView::TagTreeView(bool editable,  QWidget* parent)
...
inline void TagTreeView::init_headers(){
    // Inlined to avoid multiple definition error
...
inline void TagTreeView::place_tags(uint64_t root){
    // Inlined to avoid multiple definition error
...
inline void TagTreeView::dragMoveEvent(QDragMoveEvent* e){
    // Inlined to avoid multiple definition error
    TagTreeModel* mdl = (TagTreeModel*)this->model();
