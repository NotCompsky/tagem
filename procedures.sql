...
BEGIN
    ...
    INSERT INTO tag2parent_tree (tag, parent, depth) SELECT id, id, 0 FROM _tag;
    
    DROP TABLE IF EXISTS _tmp;
    CREATE TEMPORARY TABLE _tmp LIKE tag2parent_tree;
    REPEAT
        TRUNCATE TABLE _tmp;
        INSERT INTO _tmp SELECT t2pt.tag, t2p.parent, t2pt.depth+1 FROM tag2parent_tree t2pt JOIN tag2parent t2p ON t2p.tag=t2pt.parent ON DUPLICATE KEY UPDATE depth=LEAST(_tmp.depth, t2pt.depth+1);
        INSERT INTO tag2parent_tree (tag,parent,depth) SELECT t.tag, t.parent, t.depth FROM _tmp t ON DUPLICATE KEY UPDATE depth=LEAST(tag2parent_tree.depth, t.depth);
    UNTIL ROW_COUNT() = 0
    END REPEAT;
END $$
