...
"var merge_files_status = 0;"
"var merge_files_arr;"
"function merge_files(){"
	"if(++merge_files_status == 1){"
		"merge_files_arr = get_file_ids();"
		"alert(\"Master file ID noted\\nNow deselect the master file, select all duplicates, and press the 'Merge Files' button again\");"
	"} else {"
		"merge_files_status = 0;"
		"const _file_ids = get_file_ids().split(\",\");"
		"const master_file_ids = merge_files_arr.split(\",\").filter(x => !_file_ids.includes(x));"
		"if(master_file_ids.length !== 1){"
			"alert(\"There must be exactly one master file\");"
			"return;"
		"}"
		"merge_files_arr = undefined;"
		"$.post({"
			"url:\"/merge/\"+master_file_ids[0]+\"/\"+_file_ids.join(\",\"),"
			"success:deselect_rows('#f .tbody .tr'),"
			"error:err_alert"
		"});"
	"}"
"}"
