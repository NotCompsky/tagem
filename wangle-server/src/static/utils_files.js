...
"function hide_all_views_except(except){"
	"for(let type of ['img','video','audio','iframe','object']){"
		"if(type === except){"
			"unhide('view-'+type);"
			"continue;"
		"}"
...
"function set_file_view_src(type, src, _mimetype){"
	...
	"if(playlist_file_ids!==undefined){"
		"x.parentNode.addEventListener('ended', playlist_listener);"
	"}"
"}"
"function play(type, src, _mimetype){"
	...
	"if(playlist_file_ids!==undefined){"
		"player.parentNode.addEventListener('ended', playlist_listener);"
	"}"
"}"

"function YTPlayer_onStateChange(e){"
	"if ((event.data !== YT.PlayerState.ENDED) || (playlist_file_ids!==undefined))"
		"return;"
	"playlist_listener();"
"}"
...
"function set_embed_html(_dir_id, _mimetype, _file_name){"
	...
	"if (embed_pre === \"\"){"
		...
		"for(var i=0; i<5; ++i){"
			"const _playlist_listener = playlist_listeners[i];"
			"if(_playlist_listener === undefined)"
				"continue;"
			"document.getElementById('view-'+playlist_listeners_types[i]).removeEventListener(_playlist_listener);"
		"}"
	"} else {"
